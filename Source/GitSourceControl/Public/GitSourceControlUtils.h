...
{
	/**
	 *  Returns an updated repo root if all selected files are in a plugin subfolder, and the plugin subfolder is a git
	 * repo This supports the case where each plugin is a sub module
	 *
	...
	/**
	 *  Returns an updated repo root if all selected file is in a plugin subfolder, and the plugin subfolder is a git
	 * repo This supports the case where each plugin is a sub module
	 *
	...
	/**
	 * Find the path to the Git binary, looking into a few places (standalone Git install, and other common tools
	 * embedding Git)
	 ...
	 * @param OutRepositoryRoot		The path to the root directory of the Git repository if found, else the path to the
	 * ProjectDir
	 * @returns true if the command succeeded and returned no errors
	 */
	 ...
	 * @param	InPathToGitBinary	The path to the Git binary
	 * @param	InRepositoryRoot	The Git repository from where to run the command - usually the Game directory (can
	 * be empty)
	 ...
	 * @returns true if the command succeeded and returned no errors
	 */
	GITSOURCECONTROL_API bool RunCommand(const FString& InCommand, const FString& InPathToGitBinary,
	 ...
	 * or for one or more files all on a same directory (by design, since we group files by directory in
	 * RunUpdateStatus())
	 *
	 * @param[in]	InPathToGitBinary	The path to the Git binary
	 * @param[in]	InRepositoryRoot	The Git repository from where to run the command - usually the Game directory
	 * (can be empty)
	 ...
	 * @param[out]	OutStates			States of files for witch the status has been gathered (distinct than InFiles in
	 * case of a "directory status")
	 */
	GITSOURCECONTROL_API void ParseStatusResults(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
	 ...
	 * @param	bMergeConflict		In case of a merge conflict, we also need to get the tip of the "remote branch"
	 * (MERGE_HEAD) before the log of the "current branch" (HEAD)
	 * @param	OutErrorMessages	Any errors (from StdErr) as an array per-line
	 ...
	 * @returns true if any states were updated
	 */
	GITSOURCECONTROL_API bool CollectNewStates(const TMap<FString, FGitSourceControlState>& InStates,
	...
	bool CollectNewStates(const TArray<FString>& InFiles, TMap<const FString, FGitState>& OutResults,
						  EFileState::Type FileState, ETreeState::Type TreeState = ETreeState::Unset,
						  ELockState::Type LockState = ELockState::Unset,
						  ERemoteState::Type RemoteState = ERemoteState::Unset);
	...
	bool GetAllLocks(const FString& InRepositoryRoot, const FString& GitBinaryFallBack,
					 TArray<FString>& OutErrorMessages, TMap<FString, FString>& OutLocks,
	...
	GITSOURCECONTROL_API bool FetchRemote(const FString& InPathToGitBinary, const FString& InPathToRepositoryRoot,
										  bool InUsingGitLfsLocking, TArray<FString>& OutResults,
										  TArray<FString>& OutErrorMessages);
	...
	GITSOURCECONTROL_API TSharedPtr<class ISourceControlRevision, ESPMode::ThreadSafe> GetOriginRevisionOnBranch(
		const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InRelativeFileName,
		TArray<FString>& OutErrorMessages, const FString& BranchName);
