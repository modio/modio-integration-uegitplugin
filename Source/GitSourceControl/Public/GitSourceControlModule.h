...
3. Trying to deactivate Git LFS 2 file locking afterward on the "Login to Revision Control" (Connect/Configure) screen
   is not working after Git LFS 2 has switched "read-only" flag on files (which needs the Checkout operation to be
editable)!
   - temporarily deactivating locks may be required if we want to be able to work while not connected (do we really need
this ???)
   - does Git LFS have a command to do this deactivation ?
	 - perhaps should we rely on detection of such flags to detect LFS 2 usage (ie. the need to do a checkout)
	   ...
	   - this would need a rework of the way the "bIsUsingFileLocking" is propagated, since this would no more be a
configuration (or not only) but a file state
...
- renaming a Blueprint in Editor leaves a redirector file, AND modify too much the asset to enable git to track its
history through renaming
- standard Editor commit dialog asks if user wants to "Keep Files Checked Out" => no use for Git or Mercurial
CanCheckOut()==false
 */
...
public:
	...
	UGitLockProviderBase* GetLockProvider() const;
	void SetLockProviderClass(TSoftClassPtr<class UGitLockProviderBase> Provider);
...
private:
	...
	void DiffAgainstOriginBranch(UObject* InObject, const FString& InPackagePath, const FString& InPackageName,
								 const FString& BranchName) const;

	void UpdateLockProviderInstance();
	...
	TStrongObjectPtr<UGitLockProviderBase> LockProvider;
