...
FGitScopedTempFile::FGitScopedTempFile(const FText& InText)
{
	...
	if (!FFileHelper::SaveStringToFile(InText.ToString(), *Filename,
...
namespace GitSourceControlUtils
{
	FString ChangeRepositoryRootIfSubmodule(TArray<FString>& AbsoluteFilePaths, const FString& PathToRepositoryRoot)
	{
		...
		for (auto& FilePath : AbsoluteFilePaths)
		{
			...
			while (!FPaths::IsSamePath(TestPath, PathToRepositoryRoot))
			{
				...
				if (TestPath.IsEmpty())
				{
					// TestPath.IsEmpty() meaning is that FilePath is not git file. So it need to removed to git command
					// file list.
					PackageNotIncludedInGit.Add(FilePath);
					UE_LOG(LogSourceControl, Warning,
						   TEXT("Package file to update has included dependent file is not git or Can't find directory "
								"path for file : %s"),
						   *FilePath);
				...
				if (FPaths::FileExists(GitTestPath) || FPaths::DirectoryExists(GitTestPath))
				{
					...
					if (!FPaths::IsSamePath(RetNormalized, PathToRepositoryRootNormalized) &&
						Ret != FPaths::GetPath(GitTestPath))
					{
	...
	bool RunCommandInternalRaw(const FString& InCommand, const FString& InPathToGitBinary,
							   const FString& InRepositoryRoot, const TArray<FString>& InParameters,
							   const TArray<FString>& InFiles, FString& OutResults, FString& OutErrors,
							   const int32 ExpectedReturnCode /* = 0 */)
	{
		int32 ReturnCode = 0;
		FString FullCommand;
		...
		if (!InRepositoryRoot.IsEmpty())
		{
			...
			if ((InFiles.Num() > 0) && !FPaths::IsRelative(InFiles[0]) && !InFiles[0].StartsWith(InRepositoryRoot))
			{
				...
				{
					RepositoryRoot = DestinationRepositoryRoot; // if found use it for the "add" command (else not, to
																// avoid producing one more error in logs)
				}
			}
		...
		// Also, Git does not have a "--non-interactive" option, as it auto-detects when there are no connected standard
...
#if PLATFORM_MAC
		// The Cocoa application does not inherit shell environment variables, so add the path expected to have git-lfs
		// to PATH
		...
		if (!bHasGitInstallPath)
		{
			...
			FullCommand =
				FString::Printf(TEXT("PATH=\"%s%s%s\" \"%s\" %s"), *GitInstallPath,
								FPlatformMisc::GetPathVarDelimiter(), *PathEnv, *InPathToGitBinary, *FullCommand);
		}
#endif
	...
	static bool RunCommandInternal(const FString& InCommand, const FString& InPathToGitBinary,
								   const FString& InRepositoryRoot, const TArray<FString>& InParameters,
								   ...
								   TArray<FString>& OutErrorMessages)
	{
		...
		bResult = RunCommandInternalRaw(InCommand, InPathToGitBinary, InRepositoryRoot, InParameters, InFiles, Results,
										Errors);
	...
	FString FindGitBinaryPath()
	{
		...
		// NOTE using only "git" (or "git.exe") relying on the "PATH" envvar does not always work as expected, depending
		// on the installation: If the PATH is set with "git/cmd" instead of "git/bin", "git.exe" launch
		// "git/cmd/git.exe" that redirect to "git/bin/git.exe" and ExecProcess() is unable to catch its outputs
		// streams. First check the 64-bit program files directory:
		...
		if (!bFound)
		{
			...
			if (PortableGitFolders.Num() > 0)
			{
				...
				GitBinaryPath = FString::Printf(TEXT("%s/GitHub/%s/cmd/git.exe"), *AppDataLocalPath,
												*(PortableGitFolders.Last())); // keep only the last PortableGit found
				bFound = CheckGitAvailability(GitBinaryPath);
				if (!bFound)
				{
					// If Portable git is not found in "cmd/" subdirectory, try the "bin/" path that was in use before
					GitBinaryPath = FString::Printf(TEXT("%s/GitHub/%s/bin/git.exe"), *AppDataLocalPath,
													*(PortableGitFolders.Last())); // keep only the last
																				   // PortableGit found
					bFound = CheckGitAvailability(GitBinaryPath);
				}
			}
		...
		if (!bFound)
		{
			...
			{
				// FindFiles just returns directory names, so we need to prepend the root path to get the full path.
				GitBinaryPath = FString::Printf(TEXT("%s/Fork/gitInstance/%s/cmd/git.exe"), *AppDataLocalPath,
												*(PortableGitFolders.Last())); // keep only the last PortableGit found
				bFound = CheckGitAvailability(GitBinaryPath);
				if (!bFound)
				{
					// If Portable git is not found in "cmd/" subdirectory, try the "bin/" path that was in use before
					GitBinaryPath = FString::Printf(TEXT("%s/Fork/gitInstance/%s/bin/git.exe"), *AppDataLocalPath,
													*(PortableGitFolders.Last())); // keep only the last
																				   // PortableGit found
					bFound = CheckGitAvailability(GitBinaryPath);
				}
...

		{
			...
			{
				NSURL* AppURL =
					[SharedWorkspace URLForApplicationWithBundleIdentifier:@"com.torusknot.SourceTreeNotMAS"];
				if (AppURL != nullptr)
				{
					NSBundle* Bundle = [NSBundle bundleWithURL:AppURL];
	...
	bool CheckGitAvailability(const FString& InPathToGitBinary, FGitVersion* OutVersion)
	{
		...
		bool bGitAvailable = RunCommandInternalRaw(
			TEXT("version"), InPathToGitBinary, FString(), FGitSourceControlModule::GetEmptyStringArray(),
		...
		if (!TokenVersionStringPtr.IsEmpty())
		{
			...
			if (Num >= 3)
			{
					...
					ParsedVersionString[2].IsNumeric())
				{
					...
					{
						UE_LOG(LogSourceControl, Log, TEXT("Git version %d.%d.%d.%s.%d.%d.%d"), OutVersion->Major,
							   OutVersion->Minor, OutVersion->Patch, *OutVersion->Fork, OutVersion->ForkMajor,
							   OutVersion->ForkMinor, OutVersion->ForkPatch);
					}
					else
					{
						UE_LOG(LogSourceControl, Log, TEXT("Git version %d.%d.%d"), OutVersion->Major,
							   OutVersion->Minor, OutVersion->Patch);
					}
				}
			}
		}
	...
	void GetUserConfig(const FString& InPathToGitBinary, const FString& InRepositoryRoot, FString& OutUserName,
					   FString& OutUserEmail)
	{
		...
		bResults = RunCommandInternal(TEXT("config"), InPathToGitBinary, InRepositoryRoot, Parameters,
									  FGitSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
	...
	bool GetBranchName(const FString& InPathToGitBinary, const FString& InRepositoryRoot, FString& OutBranchName)
	{
		...
		bResults = RunCommand(TEXT("symbolic-ref"), InPathToGitBinary, InRepositoryRoot, Parameters,
							  FGitSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
		if (bResults && InfoMessages.Num() > 0)
		{
		...
		else
		{
			...
			bResults = RunCommand(TEXT("log"), InPathToGitBinary, InRepositoryRoot, Parameters,
								  FGitSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
			if (bResults && InfoMessages.Num() > 0)
			{
	...
	bool GetRemoteBranchName(const FString& InPathToGitBinary, const FString& InRepositoryRoot, FString& OutBranchName)
	{
		...
		bool bResults = RunCommand(TEXT("rev-parse"), InPathToGitBinary, InRepositoryRoot, Parameters,
								   FGitSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
		if (bResults && InfoMessages.Num() > 0)
		{
			OutBranchName = InfoMessages[0];
		...
		if (!bResults)
		{
			...
			{
				UE_LOG(LogSourceControl, Warning,
					   TEXT("Upstream branch not found for the current branch, skipping current branch for remote "
							"check. Please push a remote branch."));
				bRunOnce = false;
			}
		}
		return bResults;
	...
	bool GetRemoteBranchesWildcard(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
								   const FString& PatternMatch, TArray<FString>& OutBranchNames)
	{
		TArray<FString> InfoMessages;
		TArray<FString> ErrorMessages;
		TArray<FString> Parameters;
		...
		if (!bResults)
		{
			...
			if (bRunOnce)
			{
				UE_LOG(LogSourceControl, Warning, TEXT("No remote branches matching pattern \"%s\" were found."),
					   *PatternMatch);
				bRunOnce = false;
			}
		}
	...
	bool GetCommitInfo(const FString& InPathToGitBinary, const FString& InRepositoryRoot, FString& OutCommitId,
					   FString& OutCommitSummary)
	{
		...
		bResults = RunCommandInternal(TEXT("log"), InPathToGitBinary, InRepositoryRoot, Parameters,
									  FGitSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
		if (bResults && InfoMessages.Num() > 0)
		{
	...
	bool GetRemoteUrl(const FString& InPathToGitBinary, const FString& InRepositoryRoot, FString& OutRemoteUrl)
	{
		...
		const bool bResults =
			RunCommandInternal(TEXT("remote"), InPathToGitBinary, InRepositoryRoot, Parameters,
							   FGitSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
		if (bResults && InfoMessages.Num() > 0)
		{
	...
	bool RunCommand(const FString& InCommand, const FString& InPathToGitBinary, const FString& InRepositoryRoot,
					...
					TArray<FString>& OutErrorMessages)
	{
		...
		if (InFiles.Num() > GitSourceControlConstants::MaxFilesPerBatch)
		{
			...
			while (FileCount < InFiles.Num())
			{
				...
				for (int32 FileIndex = 0;
					 FileCount < InFiles.Num() && FileIndex < GitSourceControlConstants::MaxFilesPerBatch;
					 FileIndex++, FileCount++)
				{
				...
				bResult &= RunCommandInternal(InCommand, InPathToGitBinary, InRepositoryRoot, InParameters,
											  FilesInBatch, BatchResults, BatchErrors);
		...
		{
			bResult = RunCommandInternal(InCommand, InPathToGitBinary, InRepositoryRoot, InParameters, InFiles,
	...
	bool RunCommit(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
				   const TArray<FString>& InParameters, const TArray<FString>& InFiles, TArray<FString>& OutResults,
				   TArray<FString>& OutErrorMessages)
	{
		bool bResult = true;
		...
		if (InFiles.Num() > GitSourceControlConstants::MaxFilesPerBatch)
		{
			...
			{
				TArray<FString> FilesInBatch;
				for (int32 FileIndex = 0; FileIndex < GitSourceControlConstants::MaxFilesPerBatch;
					 FileIndex++, FileCount++)
				{
					FilesInBatch.Add(InFiles[FileCount]);
				}
				bResult &= RunCommandInternal(TEXT("add"), InPathToGitBinary, InRepositoryRoot, AddParameters,
											  FilesInBatch, OutResults, OutErrorMessages);
				// First batch is a simple "git commit" command with only the first files
				bResult &= RunCommandInternal(TEXT("commit"), InPathToGitBinary, InRepositoryRoot, InParameters,
											  FilesInBatch, OutResults, OutErrorMessages);
			}
			...
			while (FileCount < InFiles.Num())
			{
				...
				bResult &= RunCommandInternal(TEXT("add"), InPathToGitBinary, InRepositoryRoot, AddParameters,
											  FilesInBatch, OutResults, OutErrorMessages);
				bResult &= RunCommandInternal(TEXT("commit"), InPathToGitBinary, InRepositoryRoot, Parameters,
											  FilesInBatch, BatchResults, BatchErrors);
				OutResults += BatchResults;
				OutErrorMessages += BatchErrors;
			}
		...
		{
			bResult &= RunCommandInternal(TEXT("add"), InPathToGitBinary, InRepositoryRoot, AddParameters, InFiles,
										  OutResults, OutErrorMessages);
			bResult = RunCommandInternal(TEXT("commit"), InPathToGitBinary, InRepositoryRoot, InParameters, InFiles,
										 OutResults, OutErrorMessages);
		}
	...
	public:
		FGitLfsLocksParser(const FString& InRepositoryRoot, const FString& InStatus, const bool bAbsolutePaths = true)
		{
			...
			if (Informations.Num() >= 2)
			{
				...
				// Filename ID (or we expect it to be the username, but it's empty, or is the ID, we have to assume it's
				// the current user)
	...
	public:
		FGitStatusParser(const FString& InResult)
		{
			...
			if ((IndexState == 'U' || WCopyState == 'U') || (IndexState == 'A' && WCopyState == 'A') ||
	 ...
	 *
	 * 1: The "common ancestor" of the file (the version of the file that both the current and other branch originated
	from).
	...
	*/
	...
	static void RunGetConflictStatus(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
									 const FString& InFile, FGitSourceControlState& InOutFileState)
	{
		TArray<FString> ErrorMessages;
		...
		bool bResult = RunCommandInternal(TEXT("ls-files"), InPathToGitBinary, InRepositoryRoot, Parameters, Files,
	...
	void ReloadPackages(TArray<UPackage*>& InPackagesToReload)
	{
		...
		InPackagesToReload.RemoveAll([&](UPackage* InPackage) -> bool {
			const FString PackageExtension = InPackage->ContainsMap() ? FPackageName::GetMapPackageExtension()
																	  : FPackageName::GetAssetPackageExtension();
			const FString PackageFilename =
				FPackageName::LongPackageNameToFilename(InPackage->GetName(), PackageExtension);
	 ...
	 *
	 * Called in case of a "directory status" (no file listed in the command) when using the "Submit to Revision
	 * Control" menu.
	 */
	bool ListFilesInDirectoryRecurse(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
									 const FString& InDirectory, TArray<FString>& OutFiles)
	{
		TArray<FString> ErrorMessages;
		TArray<FString> Directory;
		Directory.Add(InDirectory);
		const bool bResult =
			RunCommandInternal(TEXT("ls-files"), InPathToGitBinary, InRepositoryRoot,
							   FGitSourceControlModule::GetEmptyStringArray(), Directory, OutFiles, ErrorMessages);
	 ...
	 *
	 *  Called in case of a "directory status" (no file listed in the command) ONLY to detect Deleted/Missing/Untracked
	 * files since those files are not listed by the 'git ls-files' command.
	 *
	 * @see #ParseFileStatusResult() above for an example of a 'git status' results
	 */
	static void ParseDirectoryStatusResult(const bool InUsingLfsLocking, const TMap<FString, FString>& InResults,
	...
	/** Parse the array of strings results of a 'git status' command for a provided list of files all in a common
	directory
	 *
	 * Called in case of a normal refresh of status on a list of assets in a the Content Browser (or user selected
	...
	!! BasicCode.sln
	*/
	static void ParseFileStatusResult(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
									  const bool InUsingLfsLocking, const TSet<FString>& InFiles,
									  ...
									  TMap<FString, FGitSourceControlState>& OutStates)
	{
		...
		for (const auto& File : InFiles)
		{
			...
			if (bFound)
			{
				...
				UE_LOG(LogSourceControl, Log, TEXT("Status(%s) = '%s' => File:%d, Tree:%d"), *File, *Result,
					   static_cast<int>(StatusParser.FileState), static_cast<int>(StatusParser.TreeState));
#endif
			...
			else
			{
				...
				{
					// but also the case for newly created content: there is no file on disk until the content is saved
					// for the first time
					FileState.State.TreeState = ETreeState::NotInRepo;
#if UE_BUILD_DEBUG && GIT_DEBUG_STATUS
					UE_LOG(LogSourceControl, Log, TEXT("Status(%s) not found and does not exists => new/not controled"),
						   *File);
#endif
				}
			}
		...
		// The above cannot detect deleted assets since there is no file left to enumerate (either by the Content
		// Browser or by git ls-files)
	...
	void ParseStatusResults(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
							const bool InUsingLfsLocking, const TArray<FString>& InFiles,
							const TMap<FString, FString>& InResults, TMap<FString, FGitSourceControlState>& OutStates)
	{
		...
		for (const auto& File : InFiles)
		{
			if (FPaths::DirectoryExists(File))
			{
				...
				const bool bResult =
					ListFilesInDirectoryRecurse(InPathToGitBinary, InRepositoryRoot, File, DirectoryFiles);
				if (bResult)
				{
	...
	{
		// We can obtain a list of files that were modified between our remote branches and HEAD. Assumes that fetch has
		// been run to get accurate info.
		...
		for (auto& Branch : BranchesToDiff)
		{
			...
			const bool bResultDiff = RunCommand(TEXT("log"), InPathToGitBinary, InRepositoryRoot, ParametersLog,
												FilesToDiff, Results, ErrorMessages);
			if (bResultDiff)
			{
		...
		for (const auto& NewFile : NewerFiles)
		{
			...
			{
				FileState->State.RemoteState =
					NewFile.Value.Equals(CurrentBranchName) ? ERemoteState::NotAtHead : ERemoteState::NotLatest;
	...
	bool GetAllLocks(const FString& InRepositoryRoot, const FString& GitBinaryFallback,
					 TArray<FString>& OutErrorMessages, TMap<FString, FString>& OutLocks, bool bInvalidateCache)
	{
		...
		if (bCacheExpired)
		{
			...
			TArray<FString> ErrorMessages;
			TArray<FString> Results;
			bResult = FGitSourceControlModule::Get().GetLockProvider()->GetLockedFiles(
				InRepositoryRoot,
				FGitFileLockOpParams {GitBinaryFallback, FGitSourceControlModule::GetEmptyStringArray(), true,
...
#if UE_BUILD_DEBUG && GIT_DEBUG_STATUS
					UE_LOG(LogSourceControl, Log, TEXT("LockedFile(%s, %s)"), *LockFile.LocalFilename,
						   *LockFile.LockUser);
#endif
			...
			// We tried to invalidate the UE cache, but we failed for some reason. Try updating lock state from LFS
			// cache. Get the last known state of remote locks
			...
			else
			{
				...
				bResult = FGitSourceControlModule::Get().GetLockProvider()->GetLockedFiles(
					InRepositoryRoot,
					FGitFileLockOpParams {GitBinaryFallback, Params, false,
										  FGitSourceControlModule::GetEmptyStringArray()},
					Results, OutErrorMessages);
				for (const FString& Result : Results)
				{
	...
	bool UpdateChangelistStateByCommand()
	{
		...
		FGitSourceControlModule& GitSourceControl =
			FModuleManager::GetModuleChecked<FGitSourceControlModule>("GitSourceControl");
		FGitSourceControlProvider& Provider = GitSourceControl.GetProvider();
		if (!Provider.IsGitAvailable())
		{
			return false;
		}
		TSharedRef<FGitSourceControlChangelistState, ESPMode::ThreadSafe> StagedChangelist =
			Provider.GetStateInternal(FGitSourceControlChangelist::StagedChangelist);
		TSharedRef<FGitSourceControlChangelistState, ESPMode::ThreadSafe> WorkingChangelist =
			Provider.GetStateInternal(FGitSourceControlChangelist::WorkingChangelist);
		...
		const bool bResult = RunCommand(TEXT("--no-optional-locks status"), Provider.GetGitBinaryPath(),
										Provider.GetPathToRepositoryRoot(), Parameters, Files, Results, ErrorMsg);
	...
	bool RunUpdateStatus(const FString& InPathToGitBinary, const FString& InRepositoryRoot,
						 const bool InUsingLfsLocking, const TArray<FString>& InFiles,
						 TArray<FString>& OutErrorMessages, TMap<FString, FGitSourceControlState>& OutStates)
	{
		// Remove files that aren't in the repository
		const TArray<FString>& RepoFiles = InFiles.FilterByPredicate(
			[InRepositoryRoot](const FString& File) { return File.StartsWith(InRepositoryRoot); });
		...
		// We skip checking ignored since no one ignores files that Unreal would read in as revision controlled
		// (Content/{*.uasset,*.umap},Config/*.ini).
		TArray<FString> Results;
		// avoid locking the index when not needed (useful for status updates)
		const bool bResult = RunCommand(TEXT("--no-optional-locks status"), InPathToGitBinary, InRepositoryRoot,
										Parameters, RepoFiles, Results, OutErrorMessages);
		...
		if (bResult)
		{
			ParseStatusResults(InPathToGitBinary, InRepositoryRoot, InUsingLfsLocking, RepoFiles, ResultsMap,
							   OutStates);
		}
	...
	bool UpdateFileStagingOnSavedInternal(const FString& Filename)
	{
		...
		if (State->Changelist.GetName().Equals(TEXT("Staged")))
		{
			...
			bResult = RunCommand(TEXT("add"), Provider.GetGitBinaryPath(), Provider.GetPathToRepositoryRoot(),
								 FGitSourceControlModule::GetEmptyStringArray(), File, DummyResults, DummyMsgs);
		}
	...
	bool RunDumpToFile(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InParameter,
					   const FString& InDumpFileName)
	{
		...
		FProcHandle ProcessHandle = FPlatformProcess::CreateProc(*PathToGitOrEnvBinary, *FullCommand, bLaunchDetached,
																 bLaunchHidden, bLaunchReallyHidden, nullptr, 0,
																 *InRepositoryRoot, PipeWrite, nullptr, nullptr);
#else
		FProcHandle ProcessHandle =
			FPlatformProcess::CreateProc(*PathToGitOrEnvBinary, *FullCommand, bLaunchDetached, bLaunchHidden,
										 bLaunchReallyHidden, nullptr, 0, *InRepositoryRoot, PipeWrite);
#endif
	 ...
	 * (everything else is taken like "edit")
	 */
	static FString LogStatusToString(TCHAR InStatus)
	{
		switch (InStatus)
		{
			...
			case TEXT('A'): // added: keyword "add" to display a specific icon instead of the default "edit" action one
				return FString("add");
			case TEXT(
				'D'): // deleted: keyword "delete" to display a specific icon instead of the default "edit" action one
				return FString("delete");
			case TEXT(
				'R'): // renamed keyword "branch" to display a specific icon instead of the default "edit" action one
				return FString("branch");
			case TEXT(
				'C'): // copied keyword "branch" to display a specific icon instead of the default "edit" action one
				return FString("branch");
			case TEXT('T'):
	...
	*/
	...
	{
		TSharedRef<FGitSourceControlRevision, ESPMode::ThreadSafe> SourceControlRevision =
			MakeShareable(new FGitSourceControlRevision);
		for (const auto& Result : InResults)
		{
			if (Result.StartsWith(TEXT("commit "))) // Start of a new commit
			{
				...
				SourceControlRevision->ShortCommitId =
					SourceControlRevision->CommitId.Left(8); // Short revision ; first 8 hex characters (max that can
															 // hold a 32 bit integer)
				SourceControlRevision->CommitIdNumber = FParse::HexNumber(*SourceControlRevision->ShortCommitId);
				SourceControlRevision->RevisionNumber =
					-1; // RevisionNumber will be set at the end, based off the index in the History
			}
			...
			//	else if(Result.IsEmpty()) // empty line before/after commit message has already been taken care by
			...
			else // Name of the file, starting with an uppercase status letter ("A"/"M"...)
			{
				...
				SourceControlRevision->Action =
					LogStatusToString(Status); // Readable action string ("Added", Modified"...) instead of "A"/"M"...
				// Take care of special case for Renamed/Copied file: extract the second filename after second
				// tabulation
				int32 IdxTab;
		...
		// Then set the revision number of each Revision based on its index (reverse order since the log starts with the
		// most recent change)
		for (int32 RevisionIndex = 0; RevisionIndex < OutHistory.Num(); RevisionIndex++)
		{
			...
			// Special case of a move ("branch" in Perforce term): point to the previous change (so the next one in the
			// order of the log)
	...
	*/
	...
	bool RunGetHistory(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InFile,
					   bool bMergeConflict, TArray<FString>& OutErrorMessages, TGitSourceControlHistory& OutHistory)
	{
		bool bResults;
		{
			TArray<FString> Results;
			...
			if (bMergeConflict)
			{
				// In case of a merge conflict, we also need to get the tip of the "remote branch" (MERGE_HEAD) before
				// the log of the "current branch" (HEAD)
			...
			TArray<FString> Files;
			Files.Add(*InFile);
			bResults = RunCommand(TEXT("log"), InPathToGitBinary, InRepositoryRoot, Parameters, Files, Results,
								  OutErrorMessages);
			if (bResults)
			{
		...
		for (auto& Revision : OutHistory)
		{
			...
			bResults &= RunCommand(TEXT("ls-tree"), InPathToGitBinary, InRepositoryRoot, Parameters, Files, Results,
								   OutErrorMessages);
	...
	bool UpdateCachedStates(const TMap<const FString, FGitState>& InResults)
	{
		...
		// TODO without LFS : Workaround a bug with the Source Control Module not updating file state after a simple
		// "Save" with no "Checkout" (when not using File Lock)
	...
	bool CollectNewStates(const TMap<FString, FGitSourceControlState>& InStates,
						  TMap<const FString, FGitState>& OutResults)
	{
		if (InStates.Num() == 0)
		{
			return false;
	...
	bool CollectNewStates(const TArray<FString>& InFiles, TMap<const FString, FGitState>& OutResults,
						  EFileState::Type FileState, ETreeState::Type TreeState, ELockState::Type LockState,
	...
	bool PullOrigin(const FString& InPathToGitBinary, const FString& InPathToRepositoryRoot,
					const TArray<FString>& InFiles, TArray<FString>& OutFiles, TArray<FString>& OutResults,
					TArray<FString>& OutErrorMessages)
	{
		if (FGitSourceControlModule::Get().GetProvider().bPendingRestart)
		{
			FText PullFailMessage(
				LOCTEXT("Git_NeedBinariesUpdate_Msg",
		...
		// Get the list of files which will be updated (either ones we changed locally, which will get potentially
		// rebased or merged, or the remote ones that will update)
		TArray<FString> DifferentFiles;
		const bool bResultDiff =
			RunCommand(TEXT("diff"), InPathToGitBinary, InPathToRepositoryRoot, {TEXT("--name-only"), RemoteBranch},
					   FGitSourceControlModule::GetEmptyStringArray(), DifferentFiles, OutErrorMessages);
		if (!bResultDiff)
		{
			return false;
		}
		...
		if (bShouldReload)
		{
			const auto PackagesToReloadResult =
				Async(EAsyncExecution::TaskGraphMainThread, [=] { return UnlinkPackages(Files); });
	...
	TSharedPtr<ISourceControlRevision, ESPMode::ThreadSafe> GetOriginRevisionOnBranch(const FString& InPathToGitBinary,
																					  const FString& InRepositoryRoot,
																					  const FString& InRelativeFileName,
																					  TArray<FString>& OutErrorMessages,
																					  const FString& BranchName)
	{
		...
		TArray<FString> Files;
		const auto bResults =
			RunCommand(TEXT("show"), InPathToGitBinary, InRepositoryRoot, Parameters, Files, Results, OutErrorMessages);
